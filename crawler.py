...
class Crawler:
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # Removed the forced 5-second Auto-Proxy discovery delay here!
            if self.cfg.proxies:
                await self._emit("log", level="ok", msg=f"Using {len(self.cfg.proxies)} manual proxies")
            ...
            await self._emit("log", level="ok", msg=f"Phase 1 complete — {len(all_urls)} URLs discovered")
            ...
            await self._emit("log", level="ok", msg=f"Phase 2 complete — {ok} pages OK, {errs} errors")
            ...
            if self.cfg.dl_images or self.cfg.dl_videos or self.cfg.dl_gifs:
                ...
                for r in self._results:
                    for url in r.get("media_urls", []):
                        ...
                        if ext == ".gif":
                            if self.cfg.dl_gifs: all_media.add(url)
                        elif ext in {".mp4", ".webm", ".ogg", ".mov", ".avi", ".mkv", ".m4v"}:
                            if self.cfg.dl_videos: all_media.add(url)
                        else:
                            # Treat standard image extensions + extensionless urls as Images
                            if self.cfg.dl_images: all_media.add(url)
                            
                media_list = list(all_media)
                if media_list:
                    await self._emit("phase", phase=3, text=f"Downloading {len(media_list)} media files…")
    ...
    async def _bfs_discover(self) -> Dict[str, int]:
        ...
        while queue and len(visited) < self.cfg.max_pages and not self._stop:
            ...
            results = await asyncio.gather(*[discover_one(u, d) for u, d in batch], return_exceptions=True)

            for (url, depth), res in zip(batch, results):
                if isinstance(res, Exception): continue
                _, links = res
                for link in links:
                    if link not in visited and len(visited) < self.cfg.max_pages and self._is_allowed(link):
                        visited[link] = depth + 1
                        queue.append((link, depth + 1))

            await self._emit("bfs_progress", found=len(visited), queue=len(queue), pct=min(100, int(len(visited) / self.cfg.max_pages * 100)))

        return visited

    async def _fetch_links(self, url: str) -> List[str]:
        try:
            headers = {"User-Agent": random.choice(self.cfg.user_agents), "Accept-Encoding": "gzip, deflate"}
            proxy   = self._get_proxy()
            async with self._session.get(url, headers=headers, proxy=proxy, allow_redirects=True) as resp:
    ...
    async def _parallel_fetch(self, urls: List[str]):
        ...
        async def worker():
            ...
            while not q.empty():
                ...
                await self._emit("page_done", url=url, status=result["status"], ms=result["response_ms"],
                                 success=result["success"], error=result.get("error", ""), done=done, total=total, pct=int(done / total * 100))
                q.task_done()

        workers = [asyncio.create_task(worker()) for _ in range(self.cfg.max_concurrent)]
        await asyncio.gather(*workers)

    async def _full_fetch(self, url: str) -> Dict[str, Any]:
        result: Dict[str, Any] = {"url": url, "success": False, "status": 0, "response_ms": 0, "error": "", "media_urls": []}
        try:
            headers = {"User-Agent": random.choice(self.cfg.user_agents), "Accept-Encoding": "gzip, deflate"}
            proxy   = self._get_proxy()
            ...
            try:
                ...
                if proxy: self._mark_proxy_failed(proxy)
                raise
    ...
    async def _download_media(self, media_urls: List[str]):
        ...
        for u in media_urls: q.put_nowait(u)
                ...
                try:
                    ...
                    async with self._session.get(url, headers=headers, proxy=proxy) as resp:
                        if resp.status == 200:
                            ...
                            await self._emit("media_done", url=url, done=done, total=total, pct=int(done / total * 100))
                except Exception:
                    pass
                q.task_done()
    ...
    async def _finish(self, t0: float, stopped: bool = False):
        ...
        await self.output.write_report(
            self._results,
            {"base_url": self.cfg.base_url, "elapsed": elapsed, "media": self._media_count},
        )
        await self._emit("done", stopped=stopped, pages=ok, total=len(self._results),
                         media=self._media_count, bytes=self._bytes, elapsed=round(elapsed, 2))
    ...
    def _is_allowed(self, url: str) -> bool:
        ...
        if self.cfg.same_domain_only and not same_domain(url, self.base_host): return False
        ...
        if ext:
            ...
            if is_gif: return self.cfg.dl_gifs
            if is_vid: return self.cfg.dl_videos
            if is_img: return self.cfg.dl_images
            
            if ext not in self.cfg.allowed_extensions:
                return False
                
        for pat in self.cfg.exclude_patterns:
            if pat.lower() in url.lower(): return False
        if self._robots and not self._robots.can_fetch("*", url): return False
        return True

    async def _load_robots(self):
        robots_url = f"{urlparse(self.cfg.base_url).scheme}://{self.base_host}/robots.txt"
        try:
            async with self._session.get(robots_url, headers={"Accept-Encoding": "gzip, deflate"}) as resp:
                if resp.status == 200:
                    ...
                    self._robots = rp
                    await self._emit("log", level="dim", msg=f"robots.txt loaded from {robots_url}")
        except Exception:
            pass

    def _get_proxy(self) -> Optional[str]:
        if not self.cfg.proxies: return None
        attempts, max_attempts = 0, len(self.cfg.proxies)
        while attempts < max_attempts:
            p = self.cfg.proxies[self._proxy_idx % len(self.cfg.proxies)]
            self._proxy_idx += 1
            if p not in self._failed_proxies: return p
            attempts += 1
    ...
    def _mark_proxy_failed(self, proxy: Optional[str]):
        if proxy: self._failed_proxies.add(proxy)
